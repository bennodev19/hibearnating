    ...
    >
      ...
      <ContentContainer>
        ...
        <InfoText>
          {isLoading
            ? 'Loading..'
            : 'Drag your Okay Bear here and let him hibearnate.'}
        </InfoText>
      </ContentContainer>
    </Container>
  );
...
const Container = styled.div<{ isDragActive: boolean; disabled: boolean }>`
  ...
  background: ${({ isDragActive }) => (isDragActive ? '#FFF9EF' : '#FAF4E8')};
  border: ${({ isDragActive }) => (isDragActive ? '4px' : '2px')} solid
    ${({ isDragActive }) => (isDragActive ? '#19AB6D' : '#B99673')};
  border-radius: 5px;
  cursor: pointer;
  filter: drop-shadow(
